...
function [X,FO,In]=recombinacion( des, X, FO, In, N, t, c, v, b, pr)

% [des,X,FO,In,N,t,c,v,b]=seleccion4 ;  %funcion que llama a los valores anteriores
% N
Xh=zeros(N,t);  


%% proceso de organización de hijos (en desorden)
% i=0;    % se desplaza en el vector de descendientes 
% o=0;    % se desplaza en la matriz de nuevos hijos
% while i<N    % mientras no se alcancel el número de hijos
%     i=i+1;   % incrementar la variable que se desplaza en el vector que se definen el numero de descendientes
%     
%    if des(i,1)==0   % si no se definen descendientes para ese padre (que no se haga nada)
%    else            % si hay descendientes para ese padre 
%        j=0;        % inicializar la variable que repite al padre
%        
%      while j<des(i,1)   % mientras la variable que repita al padre sea menor que el numero de descendientes
%          j=j+1;   % incrementar la variable que define que ya se puso un descendiente en la matriz Xh
%         o=o+1;    %incrementar el contador de espacios ocupados en la matriz o
%       Xh(o,:)=X(i,:);   % se copia al hijo determinado en la matriz Xh
%      end
%    end
% end
