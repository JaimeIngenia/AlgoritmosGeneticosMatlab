...
bres=bnuevo/2;        %   volumen el cual será la nueva restricción de los dos volumenes
...
x1=zeros((size(OrdenA,1)),1);             % variable que indica si el producto entra o está fuera de la mochila
k1=0;                                   % agregador de productos
while k1< size(OrdenA,1)                 % recorrer todos los productos    
...
%% el índice de sensibilidad será el volumen 

OrdenB=zeros(1,3);

for i=1:1:size(Ub,1)      %ordenamiento debido al indice de sensibilidad 
[s,j]=min(Ub(:,2));           %ordenamiento debido al  volumen 
...
x2=zeros(size(OrdenB,1),1);             % variable que indica si el producto entra o está fuera de la mochila
k2=0;                                  % agregador de productos
while k2< size(OrdenB,1)                 % recorrer todos los productos    
...
TotalCosto=costo1+costo2+sum(atractivos(:,1))
totalVolumen=volumen1+volumen2+sum(atractivos(:,2))
